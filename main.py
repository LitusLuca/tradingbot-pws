...
def ChangingTraining(instrument, episodes, episodeTimeSpan, minDate, maxDate, iteration):
    ...
    agent = TradingAgent(environment.inputSpace, environment.actionSpace, safeFile="./models/changing-{}-{}.h5".format(instrument, iteration), epsilonDecay=0.97, gamma=0.9, epsilonMinimum=0.1)
    agent.load()
    for E in range(episodes):
        print("start episode: {:d}/{:d}, exploration: {:.2f}".format(E+1, episodes, agent.exploration))
        print("--------------------\n")
